...
======================================
Updated: X-Axis now explicitly shows natural numbers (1, 2, 3, 4...) 
using MultipleLocator with dynamic overlap prevention.
"""
...
import numpy as np
import matplotlib.ticker as ticker  # Used for MultipleLocator and MaxNLocator
...
def calculate_riemann_zeta(s: complex, terms: int = 1000) -> Tuple[Optional[complex], Optional[str], bool]:
    if s == 1 + 0j: return None, None, False
    ...
    if s.real > 1:
        ...
        for n in range(1, terms + 1):
            term = n**(-s)
            zeta_sum += term
            if n <= 5: series_parts.append(f"{n}^(-{s})")
        ...
        for n in range(1, terms + 1):
            term = ((-1)**(n-1)) * (n**(-s))
            eta_sum += term
            if n <= 5: series_parts.append(f"((-1)^{n-1}) * {n}^(-{s})")
        denominator = 1 - (2**(1-s))
        ...
        term1, term2, term3 = 2**s, (complex(math.pi)**(s-1)), cmath.sin((math.pi * s) / 2)
...
class FibPrimeApp:
    COLORS = {
        "primary": "#2c3e50", "secondary": "#3498db", "save_accent": "#2980b9",
        "fib_accent": "#e67e22", "prime_accent": "#27ae60", "zeta_accent": "#8e44ad",
        "danger": "#e74c3c", "text_dark": "#2f3640", "bg_light": "#ffffff",
        "table_header": "#ecf0f1"
    }
    ...
    def __init__(self, root: tk.Tk):
        ...
        self.last_gap_summary_data: Dict[int, List[str]] = {}
        self.gap_view_mode = "Table"
        ...
        self.content_frame = tk.Frame(
            ...
            padx=30, pady=30, highlightthickness=1, highlightbackground="#dcdde1"
        )
    ...
    def setup_navigation(self) -> None:
        ...
        for text, cmd, side in nav_buttons:
            btn = tk.Button(self.nav_bar, text=text, command=cmd, bg=self.COLORS["primary"], 
                            fg="white", relief="flat", font=("Segoe UI", 10, "bold"), padx=15, cursor="hand2")
    ...
    def create_result_display(self) -> tk.Text:
        ...
        text_area = tk.Text(container, wrap="none", font=("Consolas", 11), bg="#f9f9f9", 
    ...
    def show_fib_screen(self) -> None:
        ...
        self.btn_plot = tk.Button(btn_frame, text="Visualize", bg=self.COLORS["secondary"], 
                                  ...
                                  padx=15, cursor="hand2", state="disabled", command=lambda: self.plot_data("Fibonacci"))
    ...
    def show_prime_screen(self) -> None:
        self.clear_frame()
        tk.Label(self.content_frame, text="Prime Number Distribution Analysis", font=("Segoe UI", 16, "bold"), 
                 bg=self.COLORS["bg_light"], fg=self.COLORS["primary"]).pack(pady=(0, 20))
        ...
        tk.Button(gen_group, text="Generate Primes", bg=self.COLORS["prime_accent"], fg="white", 
                  font=("Segoe UI", 10, "bold"), relief="flat", padx=15, cursor="hand2", 
                  command=lambda: self.start_thread(self.exec_prime)).pack(side="left", padx=5)
        tk.Button(gen_group, text="Prime Gap", bg=self.COLORS["primary"], fg="white", 
                  font=("Segoe UI", 10, "bold"), relief="flat", padx=15, cursor="hand2", 
                  command=lambda: self.start_thread(self.exec_prime_gap)).pack(side="left", padx=5)
        tk.Button(gen_group, text="Gap Summary", bg=self.COLORS["primary"], fg="white", 
                  font=("Segoe UI", 10, "bold"), relief="flat", padx=15, cursor="hand2", 
                  command=lambda: self.start_thread(self.exec_prime_gap_summary)).pack(side="left", padx=5)
        action_group = tk.Frame(main_btn_frame, bg=self.COLORS["bg_light"])
        action_group.pack(side="left", padx=10)
        self.btn_plot = tk.Button(action_group, text="Visualize", bg=self.COLORS["secondary"], 
                                  fg="white", font=("Segoe UI", 10, "bold"), relief="flat", 
                                  padx=15, cursor="hand2", state="disabled", command=lambda: self.plot_data("Prime"))
        self.btn_plot.pack(side="left", padx=5)
        self.btn_save = tk.Button(action_group, text="Save to CSV", bg=self.COLORS["save_accent"], 
                                  fg="white", font=("Segoe UI", 10, "bold"), relief="flat", 
                                  padx=15, cursor="hand2", state="disabled", 
                                  command=lambda: self.export_to_csv("prime", ["Index N", "Prime Number"]))
        self.btn_save.pack(side="left", padx=5)
        self.btn_toggle_view = tk.Button(action_group, text="Switch to Graph", bg="#7f8c8d", fg="white", 
                                         font=("Segoe UI", 10), relief="flat", padx=15, cursor="hand2", 
                                         state="disabled", command=self.toggle_gap_view)
        self.btn_toggle_view.pack(side="left", padx=5)
        self.result_label = self.create_result_display()
    ...
    def show_zeta_screen(self) -> None:
        ...
        tk.Label(self.content_frame, text="Riemann Zeta Function", font=("Segoe UI", 16, "bold"), 
                 bg=self.COLORS["bg_light"], fg=self.COLORS["primary"]).pack(pady=(0, 20))
        self.entry_n = self.create_input_field("Enter complex numbers s (separated by , or ;):")
        btn_frame = tk.Frame(self.content_frame, bg=self.COLORS["bg_light"])
        btn_frame.pack(pady=20)
        ...
        self.btn_zeta_plot_2d = tk.Button(btn_frame, text="Visualize 2D", bg=self.COLORS["secondary"], 
                                         fg="white", font=("Segoe UI", 10, "bold"), relief="flat", 
                                         padx=15, cursor="hand2", state="disabled", command=lambda: self.plot_zeta_complex())
        ...
        self.btn_zeta_plot_3d = tk.Button(btn_frame, text="Visualize 3D", bg=self.COLORS["secondary"], 
                                         fg="white", font=("Segoe UI", 10, "bold"), relief="flat", 
                                         padx=15, cursor="hand2", state="disabled", command=lambda: self.plot_zeta_3d())
    ...
    def export_to_csv(self, prefix: str, headers: List[str]) -> None:
        ...
        except Exception as e: messagebox.showerror("Export Error", f"Could not save file: {e}")
    ...
    def exec_fib(self) -> None:
        ...
        except ValueError: self.root.after(0, lambda: messagebox.showerror("Error", "Invalid integer."))
    ...
    def exec_prime_gap(self) -> None:
        try:
            ...
            if len(primes) < 2:
                self.root.after(0, lambda: messagebox.showinfo("Info", "Not enough primes."))
                return
            gap_strings = [f"{i+1}){primes[i+1]}-{primes[i]}={primes[i+1]-primes[i]}" for i in range(len(primes)-1)]
            self.root.after(0, self.display_result, self.format_prime_gaps(gap_strings))
        except ValueError: self.root.after(0, lambda: messagebox.showerror("Error", "Invalid integer."))
    ...
    def exec_prime_gap_summary(self) -> None:
        try:
            ...
            for i in range(len(primes) - 1):
                ...
                gap_map[p2 - p1].append(f"({p1}, {p2})")
            self.last_gap_summary_data = gap_map
            self.gap_view_mode = "Table"
            self.root.after(0, self.render_gap_summary)
            self.root.after(0, lambda: self.btn_toggle_view.config(state="normal", text="Switch to Graph"))
        except ValueError: self.root.after(0, lambda: messagebox.showerror("Error", "Invalid integer."))

    def toggle_gap_view(self) -> None:
        self.gap_view_mode = "Graph" if self.gap_view_mode == "Table" else "Table"
        self.btn_toggle_view.config(text="Switch to Table" if self.gap_view_mode == "Graph" else "Switch to Graph")
        self.render_gap_summary()

    def render_gap_summary(self) -> None:
        if self.gap_view_mode == "Table": self.display_gap_table()
        else: self.display_gap_graph()

    def display_gap_table(self) -> None:
        gap_map = self.last_gap_summary_data
        sorted_gaps = sorted(gap_map.keys())
        COL_GAP, COL_PAIRS, COL_TOTAL = 12, 60, 15
        ...
        for gap in sorted_gaps:
            ...
            table_rows.append(f" {gap:<{COL_GAP}} | {row_lines[0]:<{COL_PAIRS}} | Total: {total_count:<{COL_TOTAL-7}} ")
            for j in range(1, len(row_lines)):
                table_rows.append(f" {' ' * COL_GAP} | {row_lines[j]:<{COL_PAIRS}} | {' ' * (COL_TOTAL-1)} ")
            table_rows.append(" " * (COL_GAP + 3) + "┈" * (COL_PAIRS + COL_TOTAL + 2))
        self.display_result(header + "\n" + separator + "\n" + "\n".join(table_rows))

    def display_gap_graph(self) -> None:
        if not self.last_gap_summary_data: return
        self.display_result("[Graph View Active - See External Window]")
        gap_map = self.last_gap_summary_data
        sorted_gaps = sorted(gap_map.keys())
        counts = [len(gap_map[g]) for g in sorted_gaps]
        plt.style.use('seaborn-v0_8-muted')
        fig, ax = plt.subplots(figsize=(12, 7))
        bars = ax.bar(sorted_gaps, counts, color=self.COLORS["prime_accent"], edgecolor='black', alpha=0.8)
        ax.set_title("Prime Gap Distribution (Histogram)", fontsize=14, fontweight='bold')
        ax.set_xlabel("Gap Size (n)", fontsize=12)
        ax.set_ylabel("Number of Prime Pairs", fontsize=12)
        
        # FIX: Show ALL natural numbers if range is small, otherwise use a readable integer scale
        max_gap = max(sorted_gaps) if sorted_gaps else 0
        if max_gap <= 30:
            ax.xaxis.set_major_locator(ticker.MultipleLocator(1))
        else:
            ax.xaxis.set_major_locator(ticker.MaxNLocator(integer=True))
        
        # Rotate ticks to prevent overlap when many natural numbers are shown
            ...
            if event.inaxes == ax:
                for bar in bars:
                    if bar.contains(event)[0]:
                        gap_val = int(bar.get_x() + bar.get_width()/2)
                        pairs_list = gap_map.get(gap_val, [])
                        if popup is None or not popup.winfo_exists():
                            popup = tk.Toplevel(self.root)
                            popup.title(f"Pairs for Gap {gap_val}")
                            popup.geometry("300x250")
                            popup.configure(bg="white")
                            txt = tk.Text(popup, font=("Consolas", 10), padx=10, pady=10, wrap="word")
                            txt.pack(side="left", expand=True, fill="both")
                            sb = tk.Scrollbar(popup, command=txt.yview)
                            sb.pack(side="right", fill="y")
                            txt.config(yscrollcommand=sb.set)
                        popup.deiconify()
                        popup.geometry(f"+{int(event.x + 50)}+{int(event.y + 50)}")
                        txt = popup.winfo_children()[0]
                        txt.config(state="normal")
                        txt.delete("1.0", tk.END)
                        txt.insert(tk.END, f"Total Pairs: {len(pairs_list)}\n\n" + "\n".join(pairs_list))
    ...
    def exec_zeta(self) -> None:
        try:
            ...
            for idx, s_str in enumerate(s_strings, 1):
                try:
                    ...
                    if converges:
                        line = f"{idx}. ζ({s_str}) = {val if series != 'Trivial Zero' else '0'}\n   {series}\n"
                        if val is not None: self.zeta_pairs.append((s_val, val))
                    else: line = f"{idx}. ζ({s_str}) = Diverges\n   Pole at s=1\n"
                    all_results.append(line)
                except ValueError: all_results.append(f"{idx}. Error: '{s_str}' is invalid.\n")
        ...
        except Exception as e: self.root.after(0, lambda: messagebox.showerror("Error", f"Error: {e}"))
    ...
    def plot_data(self, mode: str) -> None:
        ...
        if len(all_x) > 5000:
            ...
            x_plot, y_plot = all_x[indices], all_y[indices]
        else:
    ...
    def plot_zeta_complex(self) -> None:
        ...
        s_reals, s_imags = [p[0].real for p in self.zeta_pairs], [p[0].imag for p in self.zeta_pairs]
        z_reals, z_imags = [p[1].real for p in self.zeta_pairs], [p[1].imag for p in self.zeta_pairs]
        ...
        ax.set_xlabel("Real Part (Re)", fontsize=12); ax.set_ylabel("Imaginary Part (Im)", fontsize=12)
        ax.legend(); ax.grid(True, linestyle=':', alpha=0.6); ax.set_aspect('equal')
        plt.tight_layout(); plt.show()
    ...
    def plot_zeta_3d(self) -> None:
        ...
        s_reals, s_imags, z_mags = [p[0].real for p in self.zeta_pairs], [p[0].imag for p in self.zeta_pairs], [abs(p[1]) for p in self.zeta_pairs]
        ...
        ax.set_xlabel("Real Part (Re s)", fontsize=12); ax.set_ylabel("Imaginary Part (Im s)", fontsize=12); ax.set_zlabel(r"$|\zeta(s)|$", fontsize=12)
        ax.grid(True, linestyle='--', alpha=0.5); ax.view_init(elev=30, azim=45)
        plt.tight_layout(); plt.show()
    ...
    def show_about(self) -> None:
        about_text = (
            ...
            "- Prime Gap: The difference between two consecutive primes p(n+1) - p(n).\n"
            "- Theorems: The Twin Prime Conjecture posits that there are infinitely many \n"
            "  primes with a gap of 2. Zhang's Theorem (2013) proved that there is a \n"
            "  finite bound that infinitely many prime gaps do not exceed.\n"
            "- Gap Summary: Professional frequency analysis and visualization.\n\n"
